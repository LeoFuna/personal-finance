...
export function SideMenu() {
  return (
    <aside
      id="default-sidebar"
      className="w-64 h-full text-blue transition-transform -translate-x-full sm:translate-x-0"
      aria-label="Sidebar"
    >
      <div className="flex flex-col justify-between h-full py-4 overflow-y-auto">
        <ul className="space-y-4 font-medium">
          {MENU_OPTIONS.map(({ menuName, svg }, index) => (
            <MenuItem key={index} menuName={menuName} svg={svg} />
          ))}
        </ul>
